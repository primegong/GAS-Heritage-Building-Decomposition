...
def parse_xml(xml_file, delta=None):
		...
		try:
			...
			FocalLengthPixels = float(root.getElementsByTagName('FocalLengthPixels')[0].childNodes[0].nodeValue)
		...
		for photo in Photos:
			...
			args[image_name] = [] if image_name not in args else arg   #1_DSC00034 架次_图名 无后缀
			args[image_name] = arg
